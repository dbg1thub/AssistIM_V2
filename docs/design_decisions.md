...
## ADR-031：好友与联系人变更通过显式 contact_refresh 实时事件驱动界面刷新

- 状态：Accepted
- 决策：好友请求创建、接受、拒绝、好友删除等非聊天领域变更，通过 WebSocket 广播轻量 contact_refresh 事件；客户端实时链路把该事件桥接到 event bus，由联系人页统一触发数据刷新。
- 原因：好友请求虽然通过 HTTP 落库，但如果没有实时通知，联系人页只能依赖重新登录或手动刷新，体验和一致性都不成熟。
- 结果：聊天消息仍走消息/事件双流；联系人领域使用独立轻量刷新事件，不把好友逻辑硬塞进消息流，也不让 UI 直接监听底层 WebSocket。
