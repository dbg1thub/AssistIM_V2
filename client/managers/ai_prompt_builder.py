...
class AIPromptBuilder:
    ...
    def build_ai_chat_request(
        ...
        *,
        ...
        system_prompt = (
            ...
            "涉及本机聊天记录时，只能基于系统提供的检索材料回答，不要假装已查询。\n"
        ...
        if memory_lines:
            system_prompt += (
                "\n\n以下是本机检索到的聊天记录摘要和证据，只能用于回答用户关于历史聊天的问题；"
                "你是旁观分析者，默认做客观总结，不要逐条输出原始聊天记录；"
    ...
    def _message_context_text(self, message: ChatMessage) -> str:
        ...
        if message.message_type == MessageType.IMAGE:
            image_text = image_summary_context_text(message.extra, max_chars=self.MAX_MESSAGE_CHARS)
            return f"[图片摘要: {image_text}]" if image_text else ""
...
def reply_anchor_text(message: ChatMessage) -> str:
    ...
    if message.message_type == MessageType.FILE:
        ...
        return f"[文件内容: {name}: {file_text}]" if name else f"[文件内容: {file_text}]"
    if message.message_type == MessageType.IMAGE:
        image_text = image_summary_context_text(message.extra, max_chars=AIPromptBuilder.MAX_MESSAGE_CHARS)
        return f"[图片摘要: {image_text}]" if image_text else ""
    if message.message_type == MessageType.VOICE:
        transcript = dict((message.extra or {}).get(VOICE_TRANSCRIPT_EXTRA_KEY) or {})
        if str(transcript.get("status") or "").strip() != "ready":
