...
def test_load_default_golden_corpus_has_core_action_and_chat_cases() -> None:
    ...
    assert any("user.get" in case.expectation.required_actions for case in cases)
    assert any("friend.list" in case.expectation.required_actions for case in cases)
    assert any("message.list" in case.expectation.required_actions for case in cases)
    assert any("friend.check" in case.expectation.required_actions for case in cases)
    assert any("friend.request.list" in case.expectation.required_actions for case in cases)
    assert any("friend.request.send" in case.expectation.required_actions for case in cases)
    assert any("friend.request.accept" in case.expectation.required_actions for case in cases)
    assert any("friend.request.reject" in case.expectation.required_actions for case in cases)
    assert any("session.list" in case.expectation.required_actions for case in cases)
    assert any("session.get" in case.expectation.required_actions for case in cases)
    assert any("group.list" in case.expectation.required_actions for case in cases)
